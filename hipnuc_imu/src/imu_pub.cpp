...
void SerialIMUNode::publish_imu_data(hipnuc_raw_t *data, sensor_msgs::Imu *imu_data)
{
    ...
    {
        // ROS_WARN("data len: %d ", data->len);

        // for (int i = 0; i < data->len; i++)
        // {
        //     ROS_WARN("data->buf[%d]: %x ", i, data->buf[i]);
        // }
        // ROS_ERROR("**************************** ");

        uint32_t system_time = (uint32_t)data->buf[14] |
                               ((uint32_t)data->buf[15] << 8) |
                               ((uint32_t)data->buf[16] << 16) |
                               ((uint32_t)data->buf[17] << 24);

        // ROS_ERROR("system_time: %d ", system_time);
        // 设备启动时间（2025-01-01 12:00:00 UTC） -> UNIX 秒
        const uint32_t device_boot_time_unix = 1735689600;
        // 计算 ROS 时间戳
        uint32_t seconds = device_boot_time_unix + system_time / 1000;
        uint32_t nsecs = (system_time % 1000) * 1000000;

        ros::Time stamp(seconds, nsecs);
        imu_data->header.stamp = stamp;
        // ROS_WARN("IMU time: %.3f", stamp.toSec());
